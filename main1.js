let App = React.createClass({
  ...
  checkGameStatus(player1Array, player2Array) {
    let { winningNumbers, message } = this.state;

    let player1String = player1Array.toString();
    let player2String = player2Array.toString();
  ...
  render() {
    ...
    return (
      <div className="container">
        <h1>Tic Tac Toe</h1>
        <GameTable player={this.state.player} changePlayer={this.changePlayer} checkGameStatus={this.checkGameStatus} message={this.state.message}/>
      </div>
    )
  }
})
...
let GameTable = React.createClass ({
  ...
  addGamePiece(e){
    let { player, changePlayer, checkGameStatus } = this.props;
    ...
    if (player===1) {
      ...
      this.setState({
        player1,
      }, changePlayer(),checkGameStatus(player1, player2))
    } else {
  ...
  //   }
  //
  //   resetGameParent(player);
  // }

  render() {
    let { player, message } = this.props;
    let { player1, player2 } = this.state;
    // console.log('Child: player1 Array: ', player1, 'player2 Array: ', player2)
